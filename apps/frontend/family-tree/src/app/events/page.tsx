...
import { Calendar, Loader2, TreePine, ArrowRight } from 'lucide-react'
...
export default function EventsPage() {
  ...
  const fetchTrees = async () => {
    try {
      ...
      // Don't auto-redirect - let users choose which tree's events to view
    } catch (error) {
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Important dates and milestones in your family history
          </p>
        </div>
        ...
        {trees.length === 0 ? (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                You need to create or join a family tree to access events.
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {trees.map((tree) => (
                  <Card
                    ...
                    onClick={() => router.push(`/trees/${tree.id}/events`)}
                  >
